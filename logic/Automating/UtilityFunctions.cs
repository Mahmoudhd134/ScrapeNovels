...
public static class UtilityFunctions
{
    ...
    public static async Task Write(IEnumerable<IEnumerable<string>> chapters, string fileName, int whiteLinesBetweenLines, string fontSize, string dir)
    {
        var refactored = chapters
            .Select(c => string.Join(UtilityFunctions.Repeat("<br/>", whiteLinesBetweenLines - 1),
                c.Select(l => $"<p>{l}</p>")));
        var s = "<!DOCTYPE html>" +
                "<html><head>" +
                "<meta charset=\"UTF-8\">" +
        ...
        UtilityFunctions.ConvertToPdf(fileName, outputFileName);
    ...
    public static void ConvertToPdf(string inputName, string outputName)
    {
